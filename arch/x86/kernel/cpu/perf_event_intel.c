...
static int hsw_hw_config(struct perf_event *event)
{
	...
	if (event->attr.intx_checkpointed) {
		/*
		 * Sampling of checkpointed events can cause situations where the CPU
		 * constantly aborts because of a overflow, which is then checkpointed
		 * back and ignored. Forbid checkpointing for sampling.
		 */
