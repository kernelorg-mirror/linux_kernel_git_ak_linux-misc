...
static void update_shadow_stats(struct perf_evsel *counter, u64 *count)
{
	...
	} else if (perf_evsel__match(counter, HW_TRANSACTION, HW_TRANSACTION_START))
...
static void abs_printout(int cpu, struct perf_evsel *evsel, double avg)
{
	...
	} else if (perf_evsel__match(evsel, HW_TRANSACTION, HW_TRANSACTION_START) &&
                avg > 0 &&
	...
	} else if (perf_evsel__match(evsel, HW_TRANSACTION, HW_ELISION_START) &&
                avg > 0 && 
                runtime_cycles_intx_stats[cpu].n != 0) {
		total = avg_stats(&runtime_cycles_intx_stats[cpu]);
...
static int add_default_attributes(void)
{
	...
	if (transaction_run) {
		if (perf_evlist__add_attrs_array(evsel_list, transaction_attrs) < 0)
			return -1;
		return 0;
	}
