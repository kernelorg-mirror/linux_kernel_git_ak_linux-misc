				   ...
				   struct perf_tool *tool)
{
	...
	if (btsq->pid <= 0) {
		...
		thread = machine__find_thread(btsq->bts->machine, btsq->tid);
		if (thread)
				   ...
				   struct perf_tool *tool)
{
	...
	if (!tool->ordered_samples) {
		pr_err("Intel BTS requires ordered samples\n");
		return -EINVAL;
	}
				       ...
				       struct record_opts *opts)
{
	...
	opts->itrace_mmap_align = sizeof(struct branch);
	...
	if (opts->itrace_snapshot_mode) {
		...
		if (!opts->itrace_mmap_pages) {
			size_t sz = opts->itrace_snapshot_size + page_size - 1;
