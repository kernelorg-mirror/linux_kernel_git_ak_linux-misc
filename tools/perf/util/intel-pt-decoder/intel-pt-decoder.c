...
#define SBHASH_ORDER 14 /* 2^x cache entries */
...
static void sbhash_hit(struct intel_pt_cache *cache, struct intel_pt_sb *sb)
{
...
static void __attribute__((noinline)) sbhash_stat(struct intel_pt_cache *cache)
{
	...
	fprintf(stderr, "mem %.2f KB, hit rate %.2f total %u missed %.2f\n",
		(double)mem / 1024, (double)total_hit / cache->total_lookup,
		cache->total_lookup,
		(double)cache->missed / cache->total_lookup);
}
